...
def Test_locations(trial_num):
    ...
    def test():
        ...
        for location_number in generated_destinations.keys():
            ...
            # for each trial, randomly generate the time spent at each destination
            # also randomly generate the priority
            time = [random.randint(1, 12) for i in range(location_number)]
            ...
            for destinations in des_to_test:
                result = graph_thing.run_process(
                    time, priority, destinations, 4500, False
                )
                [rounds, path, reward, besttime, visited] = result
                total_rounds += rounds
                total_pathlen += len(path)
                total_reward += reward
                total_best_time += besttime
                total_num_reward += len(visited)
...
def Test_timeLim(trial_num, num_location):
    ...
    def test2():
        ...
        for time_limit in [3800, 4000, 4500, 5000, 5500, 6000, 6500]:
            time = [random.randint(1, 12) for i in range(num_location)]
            ...
            for destinations in des_to_test:
                result = graph_thing.run_process(
                    time, priority, destinations, time_limit, False
                )
                [rounds, path, reward, besttime, visited] = result
                total_rounds += rounds
                total_pathlen += len(path)
